...
void montana(void) {
	...
	glRotatef(20, 0, 1, 0);
	glScalef(.5, .1, .1);
	fig11.prisma3(text11.GLindex, text11.GLindex);
	glPopMatrix();
...
void carrusel(void)
{
	...
	glRotatef(-50, 0, 1, 0);
	glScalef(35.0285, 35.0285, 35.0285);
...
glRotatef(juego, 0, 1, 0);
	...
	glRotatef(90, 0, 0, 1);
	glRotatef(350, 1, 0, 0);
	fig11.cono(2, .5, 1, text11.GLindex);
	//cadena
	glPushMatrix();
	glEnable(GL_ALPHA_TEST);
	glAlphaFunc(GL_GREATER, 0.1);
...
glPopMatrix();
	...
	pintaTexto(-11, 12.0, -14.0, (void *)font, "CARNAVAL CORPSE");
